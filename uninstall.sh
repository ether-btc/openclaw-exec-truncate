#!/bin/bash
# uninstall.sh — Uninstall exec-truncate plugin from OpenClaw
...
elif [[ -f "$HOME/.openclaw/agents/main/openclaw.json" ]]; then
  OPENCLAW_CONFIG="$HOME/.openclaw/agents/main/openclaw.json"
else
  echo "❌ Error: OpenClaw config not found"
  echo "Tried locations:"
  echo "  - $HOME/.openclaw/openclaw.json"
  echo "  - $HOME/.openclaw/agents/main/openclaw.json"
  echo "  - OPENCLAW_CONFIG_PATH (if set)"
  exit 1
fi
...
echo "=== exec-truncate Plugin Uninstaller ==="
