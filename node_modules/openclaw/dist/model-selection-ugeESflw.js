...
import { i as resolveAgentConfig, o as resolveAgentEffectiveModelPrimary, u as resolveAgentModelFallbacksOverride } from "./agent-scope-CYaif3uh.js";
import { n as DEFAULT_MODEL, r as DEFAULT_PROVIDER } from "./defaults-BPcTA_Xa.js";
import { n as normalizeStaticProviderModelId, t as modelKey$1 } from "./model-ref-shared-Dh1us89S.js";
...
function resolvePluginSetupCliBackendRuntime(params) {
	...
	if (runtime) return runtime.resolvePluginSetupCliBackend(params);
...
function buildAllowedModelSet(params) {
	...
	const resolveConfiguredSyntheticEntry = (provider, model) => {
		...
		if (!Array.isArray(configuredModels)) return null;
		const configured = configuredModels.find((entry) => typeof entry?.id === "string" && entry.id.trim() === model);
		if (!configured) return null;
		return {
			provider,
			id: model,
			name: typeof configured.name === "string" && configured.name.trim().length > 0 ? configured.name.trim() : model,
			contextWindow: typeof configured.contextWindow === "number" && configured.contextWindow > 0 ? configured.contextWindow : void 0,
			reasoning: typeof configured.reasoning === "boolean" ? configured.reasoning : void 0,
			input: Array.isArray(configured.input) ? configured.input : void 0
		};
	};
	for (const raw of rawAllowlist) {
...
//#endregion
export { modelKey as C, resolveConfiguredProviderFallback as D, splitTrailingAuthProfile as E, resolveRuntimeCliBackends as O, legacyModelKey as S, parseModelRef as T, resolvePersistedSelectedModelRef as _, getModelRefStatus as a, resolveSubagentSpawnModelSelection as b, normalizeModelSelection as c, resolveConfiguredModelRef as d, resolveDefaultModelForAgent as f, resolvePersistedOverrideModelRef as g, resolvePersistedModelRef as h, buildModelAliasIndex as i, resolveAllowedModelRef as l, resolveModelRefFromString as m, buildConfiguredAllowlistKeys as n, inferUniqueProviderFromConfiguredModels as o, resolveHooksGmailModel as p, buildConfiguredModelCatalog as r, isCliProvider as s, buildAllowedModelSet as t, resolveAllowlistModelKey as u, resolveReasoningDefault as v, normalizeModelRef as w, resolveThinkingDefault as x, resolveSubagentConfiguredModelSelection as y };
