import { i as formatUncaughtError } from "./errors-CGMew-1w.js";
...
import { A as hasFlag, D as getPositiveIntFlagValue, E as getFlagValue, L as isValueToken, T as getCommandPositionalsWithRootOptions, k as getVerboseFlag } from "./logger--Y4fLUmQ.js";
import { t as resolveCliArgvInvocation } from "./argv-invocation-BeolA8Ck.js";
import { a as parseCliContainerArgs, i as maybeRunCliInContainer, n as applyCliProfileEnv, r as parseCliProfileArgs, t as normalizeWindowsArgv } from "./windows-argv-BJImglD5.js";
import { n as defaultRuntime } from "./runtime-BXaxArxm.js";
import { a as enableConsoleCapture } from "./subsystem-CYqCtHnB.js";
import { r as normalizeEnv, t as isTruthyEnvValue } from "./env-Bo5AGgpZ.js";
import { t as assertSupportedRuntime } from "./runtime-guard-N7-RsVc_.js";
import { _ as resolveStateDir } from "./paths-nQmEmngS.js";
import { o as hasMemoryRuntime } from "./memory-state-CeRc_0e1.js";
import "./logging-Bzhr7ofr.js";
import { t as ensureOpenClawCliOnPath } from "./path-env-CvVoBq9B.js";
import { i as shouldSkipPluginCommandRegistration, n as shouldRegisterPrimaryCommandOnly } from "./command-registration-policy-DRFI5HGH.js";
import { a as shouldEnsureCliPathForCommandPath, c as cliCommandCatalog, n as ensureCliExecutionBootstrap, o as resolveCliCommandPathPolicy, r as resolveCliExecutionStartupContext, s as matchesCommandPath, t as applyCliExecutionStartupPresentation } from "./command-execution-startup-CwmVup1T.js";
...
const routedCommandDefinitions = {
	health: defineRoutedCommand({
		...
		runParsedArgs: async (args) => {
			const { healthCommand } = await import("./health-DXEGq5iy.js");
	...
	status: defineRoutedCommand({
		...
		runParsedArgs: async (args) => {
			if (args.json) {
				const { statusJsonCommand } = await import("./status-json-Cykf5l8Y.js");
			...
			const { statusCommand } = await import("./status-vLc6IRTd.js");
	...
	"gateway-status": defineRoutedCommand({
		...
		runParsedArgs: async (args) => {
			const { runDaemonStatus } = await import("./status-ByAJThE1.js");
	...
	sessions: defineRoutedCommand({
		...
		runParsedArgs: async (args) => {
			const { sessionsCommand } = await import("./sessions-CLQm4sw0.js");
	...
	"agents-list": defineRoutedCommand({
		...
		runParsedArgs: async (args) => {
			const { agentsListCommand } = await import("./agents-BoWRaOYX.js");
	...
	"config-get": defineRoutedCommand({
		...
		runParsedArgs: async (args) => {
			const { runConfigGet } = await import("./config-cli-CTg9QUr4.js");
	...
	"config-unset": defineRoutedCommand({
		...
		runParsedArgs: async (args) => {
			const { runConfigUnset } = await import("./config-cli-CTg9QUr4.js");
	...
	"models-list": defineRoutedCommand({
		...
		runParsedArgs: async (args) => {
			const { modelsListCommand } = await import("./models-C4w3o6qr.js");
	...
	"models-status": defineRoutedCommand({
		...
		runParsedArgs: async (args) => {
			const { modelsStatusCommand } = await import("./models-C4w3o6qr.js");
...
async function prepareRoutedCommand(params) {
	...
	const { VERSION } = await import("./version-1ekIlWyI.js");
...
async function closeCliMemoryManagers() {
	...
	try {
		const { closeActiveMemorySearchManagers } = await import("./memory-runtime-Dttk1ggf.js");
...
async function runCli(argv = process$1.argv) {
	...
	if (shouldLoadCliDotEnv()) {
		const { loadCliDotEnv } = await import("./dotenv-D4uyldlq.js");
	...
	try {
		if (shouldUseRootHelpFastPath(normalizedArgv)) {
			const { outputPrecomputedRootHelpText } = await import("./root-help-metadata-DhcxpekB.js");
			if (!outputPrecomputedRootHelpText()) {
				const { outputRootHelp } = await import("./root-help-CCN1n1X1.js");
		...
		const [{ buildProgram }, { installUnhandledRejectionHandler }, { restoreTerminalState }] = await Promise.all([
			import("./program-DFH-8X69.js"),
			import("./unhandled-rejections-CW1k-4MC.js"),
		...
		if (primary && shouldRegisterPrimaryCommandOnly(parseArgv)) {
			const { getProgramContext } = await import("./program-context-CFJo6aTH.js");
			const ctx = getProgramContext(program);
			if (ctx) {
				const { registerCoreCliByName } = await import("./command-registry-DP7nbCtb.js");
				await registerCoreCliByName(program, ctx, primary, parseArgv);
			}
			const { registerSubCliByName } = await import("./register.subclis-DFVgmO_2.js");
		...
		if (!shouldSkipPluginCommandRegistration({
			...
			const { registerPluginCliCommandsFromValidatedConfig } = await import("./cli-CuPiwofy.js");
