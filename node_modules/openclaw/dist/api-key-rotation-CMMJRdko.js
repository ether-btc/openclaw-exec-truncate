...
function collectProviderApiKeys(provider) {
	...
	const forcedSingle = config.liveSingle ? normalizeOptionalString(process.env[config.liveSingle]) : void 0;
	if (forcedSingle) return [forcedSingle];
	const fromList = parseKeyList(config.listVar ? process.env[config.listVar] : void 0);
	const primary = config.primaryVar ? normalizeOptionalString(process.env[config.primaryVar]) : void 0;
	const fromPrefixed = config.prefixedVar ? collectEnvPrefixedKeys(config.prefixedVar) : [];
	const fallback = config.fallbackVars.map((envVar) => normalizeOptionalString(process.env[envVar])).filter(Boolean);
	const manifestFallback = getProviderEnvVars(normalizedProvider).map((envVar) => normalizeOptionalString(process.env[envVar])).filter(Boolean);
